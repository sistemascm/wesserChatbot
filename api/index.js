import { addKeyword, createFlow, createProvider, MemoryDB, createBot } from '@builderbot/bot';
...
const consultaCiudad = addKeyword('nuevo')
    .addAnswer(['¿Desde qué ciudad te estás comunicando?'], { capture: true }, async (ctx, { gotoFlow, endFlow }) => {
    ...
    .addAnswer(['Excelente, tenemos envío a domicilio en tu ciudad.', 'Nuestros productos son:', 'Botellón de Agua de 20L---$3500', 'Botellón de Agua de 12L---$2500', 'Dispenser Natural---$7000', 'Alquiler de Dispensador Frío/Calor---$17000 x mes con 4 Botellones de 20L de regalo'])
    .addAnswer(["👉 *20* para elegir Botellón de 20L", "👉 *12* para elegir Botellón de 12L", "👉 *fc* si te interesa alquilar un Frío/Calor"])
    .addAnswer(['¿En cuál estás interesado?'], { capture: true }, async (ctx, { fallBack }) => {
    const texto = ctx.body.toLocaleLowerCase();
    if ((!texto.includes('20') && !texto.includes('12') && !texto.includes('fc'))) {
        return fallBack('Disculpa, no comprendo.');
    }
})
    .addAnswer(['¡Perfecto! Déjanos a continuación tus datos (Apellido, Nombre y tu dirección)', 'En un momento te vamos a generar un acceso a nuestra app de cliente.', '¡Bienvenido a la familia de *Agua Wesser*!']);
const welcomeFlow = addKeyword(['hello', 'hola', 'buen dia', 'buen día', 'buenas tardes', 'buenas noches'])
    .addAnswer(`🙌 Hola, soy *Wesser Bot* ¿En qué te puedo ayudar hoy? `)
    .addAnswer(["Escribe 👉 *nuevo* si eres *nuevo cliente* y quieres información de nuestro servicio,", 'o solamente escríbenos tu consulta a continuación.'].join('\n'), { capture: true }, async (ctx, { endFlow }) => {
